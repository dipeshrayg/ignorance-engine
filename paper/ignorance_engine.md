...
### 5.6 Time-Machine Benchmark: flagged gaps grew *less*, not more

We froze the corpus before 2015 (78.3M abstracted works), ran the bridges detector as if operating at that point in time across the same eight fields, then measured real growth: how many new papers connecting each pair were published 2015–2026. A detector that means anything predictively should show its flagged gaps closing faster than average as research catches up to them.

It showed the opposite. The 13 pairs flagged with nonzero 2015 density averaged **731** new connecting papers by 2026; the 15 pairs already well-connected in 2015 (density 0) averaged **2,974** — roughly four times more growth, not less. The single most-flagged 2015 gap, `linguistics × pharmacology` (density 896.5), gained only 59 new papers in eleven years. Meanwhile already-thriving connections like `materials_science × robotics` (density 0.0 in 2015, already 4,974 real connecting papers) added another 4,186.

We read this as a genuine limitation, not an execution bug: established research connections compound — an existing community, funding line, and citation network keep generating output — while the small, under-connected pairs this metric flags are mostly small because they're niche, not because they're primed to explode. **Ignorance density measures static under-connection at a point in time; it does not measure trajectory, and this benchmark shows those are different things.** A metric aimed at *predicting* where research is about to accelerate would need a velocity or trend signal — comparing recent growth rate to historical growth rate, for instance — not a snapshot ratio. Full per-pair results are in `data/time_machine_results.json`.

### 5.7 Independent cross-source validation

We reran the same eight-field text-based count against Semantic Scholar's Graph API — a corpus and rate-limit pool completely independent of OpenAlex's. The two sources agree strongly at the top: the three highest-density pairs are identical and in the same order in both (`linguistics × pharmacology`, `climatology × pharmacology`, `pharmacology × robotics`), with 3 of the top 5 overlapping overall. This is real corroboration that the signal reflects something in the underlying literature rather than an artifact of one database's indexing or classification choices. Full comparison in `data/semanticscholar_text_stats.json`.

### 5.8 Plausibility filtering: partially solved, honestly bounded

We built a Wikidata-based plausibility filter (`core/plausibility.py`): resolve each field to its Wikidata entity via OpenAlex's own concept linkage, then check whether that entity's type chain reaches a recognized "branch of knowledge" root. This reliably separates genuine disciplines from the noise cases documented above — "leech biology and applications" and "apelin" both fail the check, correctly, because neither resolves to an actual field of study. It does not solve the harder case: `linguistics × materials_science` are both genuine, well-established disciplines, so a type check alone cannot flag the *pairing* as implausible. Six different Wikidata graph-distance signals were tested against this case (shared-ancestor Jaccard, shortest subclass-of path, dual-parent bridge-concept search, Wikipedia text overlap) and none discriminate it — several point the wrong way entirely, with the implausible pair scoring *closer* by some graph measures than genuine interdisciplinary connections. Taxonomic proximity, it turns out, does not track real-world research relevance. This remains open.
...
The Time-Machine Benchmark (Section 5.6) sharpens this further and in a direction we did not anticipate going in: ignorance density is a *static* signal, and it does not predict where research will accelerate next — if anything it mildly anti-correlates with subsequent growth in this sample, since already-large connections compound while small gaps mostly stay small. Combined with the granularity finding above, the honest current scope of this system is narrower than the original framing: it is a real, verifiable way to *audit* which recognizable field pairs remain under-connected right now, cross-validated across two independent literature databases (Section 5.7) — not yet a system that predicts which of those pairs will become the next real discovery. Extending it toward the latter is future work, not a claim we make here.

## 7. Limitations and Future Work

**Plausibility filtering is partially solved.** `core/plausibility.py` (Section 5.8) reliably catches candidates that aren't real fields at all — the specific failure mode that produced the "leech biology" and "apelin" noise cases. It does not catch the harder case of two genuine disciplines with an implausible pairing (`linguistics × materials_science`), and six tested approaches to that case all failed. A working solution likely needs either curated named-subfield data (does a recognized bridge term already exist for this pair?) or real abstract-text embeddings once the corpus incorporates them — not more graph-distance traversal over Wikidata's taxonomy.

**Ignorance density predicts current state, not future trajectory** (Section 5.6) — a limitation to design around rather than paper over. A velocity-based variant (recent growth rate vs. historical baseline, rather than a single snapshot ratio) is the natural next attempt, but is untested.

**Fine-grained, subfield-level search is scoped and executing.** `data/subfield_catalog.py` has all 252 OpenAlex subfields cached with domain, field, and size metadata. A 12-subfield cross-domain pool (the largest subfields from each of OpenAlex's four top-level domains) is running against the same pipeline as this paper was being finalized; results were not ready in time for this draft and will be added to the public registry directly rather than held for a revision.
